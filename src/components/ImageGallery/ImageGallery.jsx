...
const ImageGalery = ({ pictureName, pageStart }) => {
  ...
  useEffect(() => {
    ...
    setStatus('pending');
    fetch(
      `https://pixabay.com/api/?q=${pictureName}&page=${pageStart}&key=30167206-9cd8436e9cf02f01e1d7e25e7&image_type=photo&orientation=horizontal&per_page=12`
    )
      .then(response => {
        if (response.ok) {
